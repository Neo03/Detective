...
void CaptureThread::run() {
    ...
    cv::VideoCapture cap("/root/Video/Slap.mp4");
    cv::Mat tmp_frame;
        ...
        for (unsigned long i=0; i<faces.size(); i++) {
            if(isMaskOn(LANDMARKS)){
                for(unsigned long k=0; k<shapes[i].size(); k++) {
                    ...
                    //cv::putText(frame, index.toStdString(), shapes[i][k],cv::FONT_HERSHEY_SIMPLEX, 0.4, color,1);
                }
            }
            if(isMaskOn(GLASSES)){
...
void CaptureThread::drawGlasses(cv::Mat frame, vector<cv::Point2f> &marks){
    ...
    cv::Mat rotated;
    cv::warpAffine(ornament, rotated, rotateMatrix, ornament.size(), cv::INTER_LINEAR, cv::BORDER_CONSTANT, cv::Scalar(255, 255, 255));
